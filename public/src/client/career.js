...
define("forum/career", ["translator", "jquery-form"], function (translator) {
    const Career = {};
    ...
    Career.init = function () {
        ...
        function validateForm(callback) {
            ...
            validateRangedInt(
                num_programming_languages.val(),
                $("#num-programming-languages-notify"),
                1,
                5
            );
            validateRangedInt(
                num_past_internships.val(),
                $("#num-past-internships-notify"),
                0,
                4
            );
            callback();
    ...
    function validateRangedInt(value, value_notify, min_val, max_val) {
        if (!value || isNaN(value)) {
            ...
            parseInt(value, 10) < min_val ||
            parseInt(value, 10) > max_val
        ) {
            showError(
                value_notify,
                `Must be within the range [${min_val}, ${max_val}]`
            );
        } else {
            showSuccess(value_notify);
        }
