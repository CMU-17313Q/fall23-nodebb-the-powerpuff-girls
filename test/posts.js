  ...
  describe("endorse/unendorse", () => {
    ...
    it("should fail to endorse a post for a student user", async () => {
      ...
      try {
        ...
        // If the endorsement succeeds, fail the test
        assert.fail("Endorsement by a student user should fail")
      } catch (err) {
        assert.equal(
          err.message,
  ...
  describe("edit", () => {
    ...
    it("should error if title is too short", async () => {
      try {
        await apiPosts.edit(
          { uid: voterUid },
          { pid, content: "edited post content", title: "a" }
        )
      } catch (err) {
        return assert.equal(
          err.message,
    ...
    it("should error with too few tags", async () => {
      ...
      try {
        await apiPosts.edit(
          { uid: voterUid },
          { pid, content: "edited post content", tags: [] }
        )
      } catch (err) {
        assert.equal(
          err.message,
  ...
  describe("post queue", () => {
    ...
    it("should update queued post's topic if target topic is merged", async () => {
      ...
      const result1 = await apiTopics.create(
        { uid: globalModUid },
        { title: "topic A", content: "topic A content", cid }
      )
      const result2 = await apiTopics.create(
        { uid: globalModUid },
        { title: "topic B", content: "topic B content", cid }
      )
