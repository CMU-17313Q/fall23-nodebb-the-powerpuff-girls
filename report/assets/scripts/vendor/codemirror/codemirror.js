...
window.CodeMirror = (function () {
    ...
    function CodeMirror(place, options) {
        ...
        for (const opt in defaults) { if (!options.hasOwnProperty(opt) && defaults.hasOwnProperty(opt)) options[opt] = defaults[opt]; }
        ...
        operation(this, function () {
            for (const opt in optionHandlers) if (optionHandlers.propertyIsEnumerable(opt)) optionHandlers[opt](this, options[opt], Init);
    ...
    function keyMapChanged(cm) {
        const { style } = keyMap[cm.options.keyMap];
    ...
    function visibleLines(display, doc, viewPort) {
        let top = display.scroller.scrollTop; let
    ...
    function maybeUpdateLineNumberWidth(cm) {
        ...
        const { doc } = cm.view;
        const last = lineNumberFor(cm.options, doc.size - 1);
        const { display } = cm;
    ...
    function updateDisplay(cm, changes, viewPort) {
        const oldFrom = cm.display.showingFrom; const
    ...
    function updateDisplayInner(cm, changes, viewPort) {
        ...
        if (cm.options.lineNumbers && changes && changes !== true) for (var i = 0; i < changes.length; ++i) if (changes[i].diff) { positionsChangedFrom = changes[i].from; break; }
    ...
    function computeIntact(intact, changes) {
        for (let i = 0, l = changes.length || 0; i < l; ++i) {
            const change = changes[i]; const intact2 = []; const
                diff = change.diff || 0;
    ...
    function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
        ...
        let nextIntact = intact.shift(); let
            lineNo = from;
    ...
    function buildLineElement(cm, line, lineNo, dims) {
        ...
        const markers = line.gutterMarkers; const
            { display } = cm;
        ...
        if (cm.options.lineNumbers || markers) {
            ...
            if (cm.options.lineNumbers && (!markers || !markers['CodeMirror-linenumbers'])) {
                wrap.lineNumber = gutterWrap.appendChild(
                    elt('div', lineNumberFor(cm.options, lineNo), 'CodeMirror-linenumber CodeMirror-gutter-elt', `left: ${dims.gutterLeft['CodeMirror-linenumbers']}px; width: ${
    ...
    function updateSelectionCursor(cm) {
        const { display } = cm;
        const pos = cursorCoords(cm, cm.view.sel.head, 'div');
    ...
    function updateSelectionRange(cm) {
        ...
        const clientWidth = display.lineSpace.offsetWidth; const
        ...
        function drawForLine(line, fromArg, toArg, retTop) {
            ...
            const lineLen = lineObj.text.length; let
        ...
        if (sel.from.line == sel.to.line) {
            ...
            let cur = fromObj; let merged; const path = [sel.from.line, sel.from.ch]; let
                singleLine;
            while (merged = collapsedSpanAtEnd(cur)) {
            ...
            if (singleLine) {
                ...
                let middleTop; let middleBot; const
    ...
    function getStateBefore(cm, n) {
        const { view } = cm;
        let pos = findStartLine(cm, n); let
    ...
    function measureChar(cm, line, ch, data) {
        var data = data || measureLine(cm, line); let
            dir = -1;
    ...
    // First look in the cache
        const { display } = cm;
        const cache = cm.display.measureLineCache;
    ...
    function measureLineInner(cm, line) {
        const { display } = cm;
        const measure = emptyArray(line.text.length);
    ...
    function cursorCoords(cm, pos, context, lineObj, measurement) {
        ...
        const order = getOrder(lineObj); const
            { ch } = pos;
        if (!order) return get(ch);
        let main; let other; const
            linedir = order[0].level;
        for (let i = 0; i < order.length; ++i) {
            const part = order[i]; const rtl = part.level % 2; var nb; var
                here;
            if (part.from < ch && part.to > ch) return get(ch, rtl);
            const left = rtl ? part.to : part.from; const
    ...
    function coordsCharInner(cm, lineObj, lineNo, x, y) {
        ...
        let wrongLine = false; const
            cWidth = cm.display.wrapper.clientWidth;
        const measurement = measureLine(cm, lineObj);

        function getX(ch) {
            const sp = cursorCoords(cm, { line: lineNo, ch: ch }, 'line', lineObj, measurement);
            wrongLine = true;
        ...
        const bidi = getOrder(lineObj); let
            dist = lineObj.text.length;
        let from = lineLeft(lineObj); let
            to = lineRight(lineObj);
        let fromX = paddingLeft(cm.display); let
            toX = getX(to);
    ...
    function readInput(cm) {
        const { input } = cm.display;
        const { prevInput } = cm.display;
        const { view } = cm;
        const { sel } = view;
        ...
        else if (view.overwrite && posEq(from, to) && !cm.display.pasteIncoming) to = { line: to.line, ch: Math.min(getLine(cm.view.doc, to.line).text.length, to.ch + (text.length - same)) };
    ...
    function onMouseDown(e) {
        const cm = this; const { display } = cm; const { view } = cm; const { sel } = view; const
            { doc } = view;
    ...
    function onDrop(e) {
        ...
        if (files && files.length && window.FileReader && window.File) {
            const n = files.length; var text = Array(n); let
                read = 0;
            ...
            try {
                ...
                if (text) {
                    const curFrom = cm.view.sel.from; const
    ...
    let wheelSamples = 0; let wheelDX; let wheelDY; let wheelStartX; let wheelStartY; let
    ...
    function doHandleBinding(cm, bound, dropShift) {
        ...
        const { view } = cm;
        const prevShift = view.sel.shift;
        try {
    ...
    // Handle auto keymap transitions
        const startMap = getKeyMap(cm.options.keyMap); const
            next = startMap.auto;
        ...
        let name = keyNames[e_prop(e, 'keyCode')]; let
            handled = false;
        ...
        if (e_prop(e, 'shiftKey')) {
            handled = lookupKey(`Shift-${name}`, keymaps, b => doHandleBinding(cm, b, true), stop) ||
        lookupKey(name, keymaps, (b) => {
            if (typeof b === 'string' && /^go[A-Z]/.test(b)) return doHandleBinding(cm, b);
        }, stop);
        } else {
            handled = lookupKey(name, keymaps, b => doHandleBinding(cm, b), stop);
        }
    ...
    function handleCharBinding(cm, e, ch) {
        const handled = lookupKey(`'${ch}'`, allKeyMaps(cm), b => doHandleBinding(cm, b, true));
        if (handled) {
            e_preventDefault(e);
            restartBlink(cm);
        }
    ...
    function onKeyPress(e) {
        const cm = this;
        if (cm.options.onKeyEvent && cm.options.onKeyEvent(cm, addStop(e))) return;
        const keyCode = e_prop(e, 'keyCode'); const
      ...
      `border-width: 0; outline: none; overflow: hidden; opacity: .05; filter: alpha(opacity=5);`;
        ...
        function rehide() {
            ...
            if (display.input.selectionStart != null) {
                ...
                const extval = display.input.value = ` ${posEq(sel.from, sel.to) ? '' : display.input.value}`; let
                    i = 0;
                display.prevInput = ' ';
    ...
    function updateDocInner(cm, from, to, newText, selUpdate, origin) {
        ...
        const startSelFrom = view.sel.from; const
    ...
    function updateDocNoUndo(cm, from, to, lines, selUpdate, origin) {
        ...
        const nlines = to.line - from.line; const firstLine = getLine(doc, from.line); const
        ...
        if (hasHandler(cm, 'change')) {
            ...
            const changeObj = { from: from, to: to, text: lines, origin: origin };
        ...
        let newSelFrom; let newSelTo; const
            end = {
    ...
    function clipPos(doc, pos) {
        ...
        const { ch } = pos;
        const linelen = getLine(doc, pos.line).text.length;
    ...
    function skipAtomic(cm, pos, bias, mayClear) {
        ...
        search: for (;;) {
            const line = getLine(doc, curPos.line); var
                toClear;
            if (line.markedSpans) {
    ...
    function calculateScrollPos(cm, x1, y1, x2, y2) {
        const { display } = cm;
        const pt = paddingTop(display);
        y1 += pt; y2 += pt;
        const screen = display.scroller.clientHeight - scrollerCutOff; const screentop = display.scroller.scrollTop; const
            result = {};
        const docBottom = cm.view.doc.height + 2 * pt;
        const atTop = y1 < pt + 10; const
            atBottom = y2 + pt > docBottom - 10;
        if (y1 < screentop) result.scrollTop = atTop ? 0 : Math.max(0, y1);
        else if (y2 > screentop + screen) result.scrollTop = (atBottom ? docBottom : y2) - screen;

        const screenw = display.scroller.clientWidth - scrollerCutOff; const
    ...
    function changeLine(cm, handle, op) {
        let no = handle; let line = handle; const
            { doc } = cm.view;
    ...
    CodeMirror.prototype = {
        ...
        setValue: operation(null, function (code) {
            ...
            const lastLen = getLine(doc, doc.size - 1).text.length;
        ...
        getTokenAt: function (pos) {
            const { doc } = this.view;
            pos = clipPos(doc, pos);
            const state = getStateBefore(this, pos.line); const
                { mode } = this.view;
        ...
        removeLineWidget: operation(null, function (widget) {
            const ws = widget.line.widgets; const
        ...
        addWidget: function (pos, node, scroll, vert, horiz) {
            ...
            if (vert == 'over') top = pos.top;
                ...
                const vspace = Math.max(display.wrapper.clientHeight, this.view.doc.height);
        ...
        setCursor: operation(null, function (line, ch, extend) {
            const pos = clipPos(this.view.doc, typeof line === 'number' ? { line: line, ch: ch || 0 } : line);
    ...
    const modes = CodeMirror.modes = {}; const
    ...
    CodeMirror.defineMode('null', () => ({ token: function (stream) { stream.skipToEnd(); } }));
    CodeMirror.defineMIME('text/plain', 'null');

    var modeExtensions = CodeMirror.modeExtensions = {};
    CodeMirror.extendMode = function (mode, properties) {
        const exts = modeExtensions.hasOwnProperty(mode) ? modeExtensions[mode] : (modeExtensions[mode] = {});
        for (const prop in properties) { if (properties.hasOwnProperty(prop)) exts[prop] = properties[prop]; }
    };
    ...
    var commands = CodeMirror.commands = {
        ...
        killLine: function (cm) {
            const from = cm.getCursor(true); const to = cm.getCursor(false); const
                sel = !posEq(from, to);
    ...
    function markText(cm, from, to, options, type) {
        ...
        if (options) {
            for (const opt in options) { if (options.hasOwnProperty(opt)) marker[opt] = options[opt]; }
        }
        ...
        let curLine = from.line; let size = 0; let collapsedAtStart; let
            collapsedAtEnd;
        doc.iter(curLine, to.line + 1, (line) => {
            const span = { from: null, to: null, marker: marker };
    ...
    function updateMarkedSpans(oldFirst, oldLast, startCh, endCh, newText) {
        ...
        const sameLine = newText.length == 1; const
        ...
        if (!sameLine) {
            ...
            const gap = newText.length - 2; let
                gapMarkers;
    ...
    function hlSpans(val) {
        ...
        const spans = val.markedSpans; let
            out = null;
        for (let i = 0; i < spans.length; ++i) {
    ...
    function makeLine(text, markedSpans, height) {
        const line = { text: text, height: height };
    ...
    function highlightLine(cm, line, state) {
        const { mode } = cm.view;
        const { flattenSpans } = cm.options;
        let changed = !line.styles; let pos = 0; let curText = ''; let
            curStyle = null;
        const stream = new StringStream(line.text, cm.options.tabSize); const
            st = line.styles || (line.styles = []);
        if (line.text == '' && mode.blankLine) mode.blankLine(state);
        while (!stream.eol()) {
            const style = mode.token(stream, state); const
    ...
    function lineContent(cm, realLine, measure) {
        let merged; let line = realLine; let lineBefore; let sawBefore; let
            simple = true;
    ...
    function buildToken(builder, text, style, startStyle, endStyle) {
        ...
        if (!tokenSpecialChars.test(text)) {
            ...
            var content = document.createDocumentFragment(); let
                pos = 0;
            while (true) {
    ...
    function buildTokenMeasure(builder, text, style, startStyle, endStyle) {
        ...
        buildToken(builder, text.charAt(i), style, i == 0 && startStyle, i == text.length - 1 && endStyle);
        }
    ...
    function insertLineContent(line, builder) {
        ...
        const allText = line.text; const
            len = allText.length;
        let pos = 0; var i = 0; let text = ''; let
            style;
        let nextChange = 0; let spanStyle; let spanEndStyle; let spanStartStyle; let
            collapsed;
        for (;;) {
    ...
    BranchChunk.prototype = {
        ...
        remove: function (at, n, callbacks) {
            ...
            for (let i = 0; i < this.children.length; ++i) {
                const child = this.children[i]; const
                    sz = child.chunkSize();
                if (at < sz) {
    ...
    function addChange(cm, start, added, old, origin, fromBefore, toBefore, fromAfter, toAfter) {
        const { history } = cm.view;
        history.undone.length = 0;
        const time = +new Date();
        let cur = lst(history.done);
        ...
        (history.lastOp == cm.curOp.id ||
         ...
         history.lastTime > time - 600)) {
            ...
            if (last.start > start + old.length || last.start + last.added < start) {
                ...
                const endAfter = Math.max(0, (start + old.length) - (last.start + last.added));
    ...
    function signalLater(cm, emitter, type /* , values... */) {
        const arr = emitter._handlers && emitter._handlers[type];
        if (!arr) return;
        const args = Array.prototype.slice.call(arguments, 3); const
    ...
    // Character types for codepoints 0 to 0xff
        ...
        return function charOrdering(str) {
            ...
            const len = str.length; const types = []; let
                startType = null;
