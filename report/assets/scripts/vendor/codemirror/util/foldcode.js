...
CodeMirror.braceRangeFinder = function (cm, start) {
    ...
    let at = lineText.length; let startChar; let
        tokenType;
    for (;;) {
    ...
    let count = 1; const lastLine = cm.lineCount(); let end; let
        endCh;
    outer: for (let i = line + 1; i < lastLine; ++i) {
        const text = cm.getLine(i); let
            pos = 0;
        for (;;) {
            let nextOpen = text.indexOf('{', pos); let
...
CodeMirror.indentRangeFinder = function (cm, start) {
    const tabSize = cm.getOption('tabSize'); const
...
CodeMirror.newFoldFunction = function (rangeFinder, widget) {
    ...
    return function (cm, pos) {
        ...
        const present = cm.findMarksAt(range.from); let
            cleared = 0;
