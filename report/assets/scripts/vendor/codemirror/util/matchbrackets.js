(function () {
    ...
    function findMatchingBracket(cm) {
        const cur = cm.getCursor(); const line = cm.getLineHandle(cur.line); let
            pos = cur.ch - 1;
        ...
        const stack = [line.text.charAt(pos)]; const
            re = /[(){}[\]]/;
        function scan(line, lineNo, start) {
            if (!line.text) return;
            let pos = forward ? 0 : line.text.length - 1; const
    ...
    function matchBrackets(cm, autoclear) {
        ...
        const one = cm.markText(found.from, { line: found.from.line, ch: found.from.ch + 1 }, { className: style });
        const two = found.to && cm.markText(found.to, { line: found.to.line, ch: found.to.ch + 1 }, { className: style });
