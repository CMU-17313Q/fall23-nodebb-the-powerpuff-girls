(function () {
    ...
    CodeMirror.commands.newlineAndIndentContinueComment = function (cm) {
        const pos = cm.getCursor(); const
            token = cm.getTokenAt(pos);
        const { mode } = CodeMirror.innerMode(cm.getMode(), token.state);
        let space;

        if (token.type == 'comment' && mode.blockCommentStart) {
            const end = token.string.indexOf(mode.blockCommentEnd);
            const full = cm.getRange({ line: pos.line, ch: 0 }, { line: pos.line, ch: token.end }); let
                found;
