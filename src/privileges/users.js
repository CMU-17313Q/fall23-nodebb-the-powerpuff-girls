...
privsUsers.isInstructor = async function (uid) {
    // Fetch user data to check their roles or attributes
    const userData = await db.getObject(`user:${uid}`);
    console.log(userData);

    // Check if the 'accounttype' field is equal to 'instructor'
    if (userData && userData.accounttype === "instructor") {
        return true;
    }

    // If the 'accounttype' field is not 'instructor', return false
    return false;
};
